    ...
    public long countByStatus(String status){
        ...
        try{
            return em.createQuery(
            "SELECT COUNT(r) FROM Reservation r WHERE r.status=:st",Long.class).setParameter("st",status).getSingleResult();
        }finally{
                em.close();
                }
        }
    ...
    public List<Object[]> countByDayLastNDays(int days){
      EntityManager em = getEmf().createEntityManager();
      try{
          Query q = em.createNativeQuery(
            "SELECT DATE(startTime) AS d, COUNT(*) AS c " +
                "FROM reservation " +
                "WHERE status = 'APPROVED'" + " AND startTime >= DATE_SUB(CURDATE(), INTERVAL ?1 DAY) " +
